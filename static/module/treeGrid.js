    ...
    var $ = layui.$
        ...
        ,table = {
            config: {//全局配置项,表格级别
                ...
                ,cols:{//节点级别的附加字段
                    isCheckName: 'checked' //选中状态（true，false）
                    ,isRadio:'radioChecked'//单选状态（true，false）
                    ,isOpen:'open'//是否展开节点
                    ,isShow:'show'//是否显示节点
                    ,level:'level'//节点的层级关系（不需要设置）
                    ,children:'children'//存放下级的变量

                    ,cheDisabled:'cheDisabled'//禁止多选（true，false）
                    ,radDisabled:'radDisabled'//禁止单选（true，false）

                    ,iconOpen:'iconOpen'//打开的图标
                    ,iconClose:'iconClose'//关闭的图标
                    ,icon:'icon'//叶子节点图标
                }
                ,initWidth:{//默认列宽度定义
                    checkbox: 48
        ...
        if(iconOpen){
            ...
            if(isTreeNode){//父节点
                if((o[table.config.cols.iconOpen]||o[table.config.cols.iconClose])){
                    ...
                    //temTreeHtml='<i class="layui-icon layui-tree-'+o[options.idField]+' layui-icon-'+ (o[table.config.cols.isOpen] ? "right" : "down") +'" '+iconOpen+'>'+(o[table.config.cols.isOpen]?that.config.branch[1]:that.config.branch[0])+'</i>';
                }
            }else{//叶子节点
